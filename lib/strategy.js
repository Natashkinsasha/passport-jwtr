...
JWTRStrategy.prototype.authenticate = function (req, options) {
    ...
    if (!token) {
        return self.fail(new jwt.JsonWebTokenError("No auth token"));
    }


    JWTRStrategy.JwtVerifier(token, self._secretOrKey, self._verifOpts, function (jwt_err, payload) {
        if (jwt_err) {
            ...
            var verified = function (err, user, info) {
                if (err) {
                    ...
                    req.logout = function (cb) {
                        return this._jwtr.destroy(token, self._secretOrKey, self._verifOpts, cb);
                    };
                    req.logoutAll = function (cb) {
                        return this._jwtr.destroyById(user.id, self._verifOpts, cb);
                    };
